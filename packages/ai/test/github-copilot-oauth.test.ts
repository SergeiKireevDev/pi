...
describe("GitHub Copilot OAuth device flow", () => {
	...
	it("filters models to the authenticated account picker catalog", async () => {
		...
		const [pickerModel, disabledModel, hiddenModel] = provider.getModels();
		if (!pickerModel || !disabledModel || !hiddenModel) {
			throw new Error("Expected at least three GitHub Copilot models");
		}
